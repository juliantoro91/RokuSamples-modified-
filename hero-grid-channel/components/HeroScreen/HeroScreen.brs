...
sub Init()
  ...
  URLs = [
    ...
    ' "bad request",
    "https://testappjue.web.app/media4.rss",
    "https://testappjue.web.app/media8.rss",
    "https://testappjue.web.app/media12.rss",
    "https://testappjue.web.app/media.rss"
  ]
...
sub OnItemFocused()
  ...
  if itemFocused.Count() = 2 then
    ...
    itemInFocus = invalid
    if itemFocused[0] < 3
      focusedContent            = m.top.content.getChild(itemFocused[0]).clone(true)
      itemInFocus = itemFocused[1]
    else
        focusedContent = createObject("RoSGNode","ContentNode")
        for i=3 to (m.top.content.GetChildCount()-1)
            fromRowContent = m.top.content.GetChild(i)
            for j=0 to (fromRowContent.GetChildCount()-1)
                focusedContent.appendChild(fromRowContent.GetChild(j).clone(true))
            end for
        end for
        itemInFocus = (itemFocused[0]-3)*4 + itemFocused[1]
    end if
    if focusedContent <> invalid then
      m.top.focusedContent    = focusedContent
      m.top.itemInRowFocused      = itemInFocus
      m.background.uri        = focusedContent.GetChild(itemInFocus).hdBackgroundImageUrl
    end if
  end if
end sub
