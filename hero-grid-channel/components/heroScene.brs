...
sub OnRowItemSelected()
  ...
  content = m.HeroScreen.focusedContent
  itemInRowFocused = m.HeroScreen.itemInRowFocused
  m.DetailsScreen.masterContent = content
  m.DetailsScreen.content = content.GetChild(itemInRowFocused)
  m.DetailsScreen.jumpToRowItem = itemInRowFocused
  ...
  if press then
    if key = "back"
      ...
      ' if Details opened
        ...
        row = m.HeroScreen.itemFocused[0]
        item = m.DetailsScreen.jumpToRowItem
        if row >= 3
            row = m.DetailsScreen.jumpToRowItem \ 4 + 3
            item = m.DetailsScreen.jumpToRowItem MOD 4
        end if
        m.HeroScreen.jumpToRowItem=[row,item]
        m.HeroScreen.setFocus(true)
        result = true
      ' if video player opened
      else if m.HeroScreen.visible = false and m.DetailsScreen.videoPlayerVisible = true
        m.DetailsScreen.videoPlayerVisible = false
        Print "Actual videoIndex: ";m.DetailsScreen.GetChild(9).contentIndex
        result = true
      end if
    else if key = "OK"
      print "------- [ok pressed] -------"
